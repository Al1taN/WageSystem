...
namespace Ynacc.Wage.Controllers
{
    ...
    [ApiController]
    public class Imp02Controller : ControllerBase
    {
        private readonly OvertimeContext _context;
        private readonly PersonnelContext _context1;

        public Imp02Controller(OvertimeContext context, PersonnelContext context1)
        {
            _context = context;
            _context1 = context1;
        }
        ...
        [HttpPost]
        //查询数据
        public async Task<ActionResult<List<Ynacc.Wage.Dal.ImpLate02>>> GetOvertime([FromBody] JsonElement Data)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<string> ImpData02([FromBody] JsonElement Data)
        {
            try
            {
                string json = System.Text.Json.JsonSerializer.Serialize(Data);
                ...
                var oldItem = _context.ImpAddnights.Where(x => x.Year == short.Parse(item[0]["Year"].ToString()) && x.Month == short.Parse(item[0]["Month"].ToString()) && x.Deptid == item[0]["Deptid"].ToString());
                ...
                foreach (var dict in item)
                {
                    ...
                    Ynacc.Wage.Dal.ImpAddnight appinfo = new Ynacc.Wage.Dal.ImpAddnight()
                    {
                    ...
                    await _context.ImpAddnights.AddAsync(appinfo);
