...
namespace Ynacc.Wage.Controllers
{
    ...
    [ApiController]
    public class Imp01Controller : ControllerBase
    {
        private readonly LateContext _context;
        private readonly PersonnelContext _context1;

        public Imp01Controller(LateContext context, PersonnelContext context1)
        {
            _context = context;
            _context1 = context1;
        }
        ...
        [HttpPost]
        //查询数据
        public async Task<ActionResult<List<Ynacc.Wage.Dal.ImpLate>>> GetNewAttend([FromBody] JsonElement Data)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<string> ImpData01([FromBody] JsonElement Data)
        {
            try
            {
                string json = System.Text.Json.JsonSerializer.Serialize(Data);
                ...
                var oldItem = _context.ImpOtherlates.Where(x => x.Year == short.Parse(item[0]["Year"].ToString()) && x.Month == short.Parse(item[0]["Month"].ToString()) && x.Deptid == item[0]["Deptid"].ToString());
                ...
                {
                    //检测人员工资号是否与姓名匹配
                    string[] Pnames = _context1.Personnels.Where(x => x.Pid == dict["Pid"].ToString()).Select(a => a.Pname).ToArray();
                    if(Pnames[0] != dict["Pname"].ToString())
                    { return "工资号：" + dict["Pid"].ToString() + " 与姓名：" + dict["Pname"].ToString() + " 不匹配"; }

                    Ynacc.Wage.Dal.ImpOtherlate appinfo = new Ynacc.Wage.Dal.ImpOtherlate()
                    {
                    ...
                    await _context.ImpOtherlates.AddAsync(appinfo);
                    await _context.SaveChangesAsync();
                }
                transaction.Commit();
                //_context.ImpOtherlates.FromSqlInterpolated($"SET XACT_ABORT ON Begin TransAction  while {length} < {dict.Count} begin Insert Into [dbo].[imp_otherlate](deptid,pid,pname,latetype,year,month,expyear,expmonth,startdate,enddate,latedays1,latedays2,remark,maker,flag_commit,addamount,addpoint) Values({dict[length]["Deptid"].ToString()},{dict[length]["Pid"].ToString()},{dict[length]["Pname"].ToString()},{dict[length]["Latetype"].ToString()},{short.Parse(dict[length]["Year"].ToString())},{short.Parse(dict[length]["Month"].ToString())},{short.Parse(dict[length]["Expyear"].ToString())},{short.Parse(dict[length]["Expmonth"].ToString())},{short.Parse(dict[length]["Startdate"].ToString())},{short.Parse(dict[length]["Enddate"].ToString())},{short.Parse(dict[length]["Latedays1"].ToString())},{short.Parse(dict[length]["Latedays2"].ToString())},{dict[length]["Remark"].ToString()},{dict[length]["Maker"].ToString()},true,{decimal.Parse(dict[length]["Addamount"].ToString())},{decimal.Parse(dict[length]["Addpoint"].ToString())}) set {length++} end Commit TransAction");
                return "200";
            }
            catch (Exception ex)
            {
